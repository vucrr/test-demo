...
const Top = ({ status, tradeNo }: { status: number; tradeNo: string }) => {
      ...
      <div className={styles.top_box}>
        ...
        <div className={styles.button_box}>
          <Button
            ...
            onClick={() => LinkTo('/account/orderlist/0', true)}
          >
            查看订单
          </Button>
        </div>
  ...
  return (
    <div className={styles.top_box}>
      ...
      <p>离自己拥有只差一步</p>
      <div className={styles.button_box}>
        <Button className={styles.button} onClick={() => LinkTo('/account/orderlist/0', true)}>
          查看订单
        </Button>
        <Button
          className={styles.button}
          type="primary"
          onClick={() => Router.replace(`/myaccount/buy-out?trade_no=${tradeNo}`)}
        >
          重新支付
        </Button>
      </div>

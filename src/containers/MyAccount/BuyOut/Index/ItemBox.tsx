  ...
  return (
    <>
      <List className={styles.item_box}>
        <Item extra={`￥${buyout.get('buyout_price')}`}>
          <strong>买断金额</strong>
          {buyout.get('overdue_price') > 0 && <Brief>(含逾期利息¥{buyout.get('overdue_price')})</Brief>}
        </Item>
        {/* 当没有押金抵扣时，也不显示另需支付金额 */}
        {buyout.get('frozen_to_pay_price') > 0 ? (
          <Item>
            <Flex className={styles.sub_item}>
              <Flex.Item>押金抵扣</Flex.Item>
              <span>￥{buyout.get('frozen_to_pay_price')}</span>
            </Flex>
            <Flex className={styles.sub_item}>
              <Flex.Item>另需支付</Flex.Item>
              <span>￥{buyout.get('actual_buyout_price')}</span>
            </Flex>
          </Item>
        ) : (
          <></>

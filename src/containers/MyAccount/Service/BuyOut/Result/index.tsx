...
class BuyOutResult extends React.Component<BuyOutResultProps> {
  ...
  render() {
    ...
    const productBoxProps = {
      margin: true,
      thumb: result.getIn(['sku_img']),
      alias: result.getIn(['sku_name']),
      rent: `￥${result.getIn(['per_installment_price'])}`,
    }

    return (
      <Container {...containerProps}>
        <Top status={result.get('is_finish')} tradeNo={url.query.order_no} contractNo={result.get('contract_no')} />
...
const mapStateToProps = (state: any) => ({
  result: state.getIn(['myAccount', 'service', 'buyOut', 'result']),
})
